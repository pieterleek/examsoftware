...
if (!is_dir(UPLOAD_DIR)) {
    ...
    // Extra bescherming tegen uitvoeren van scripts
    @file_put_contents(
        UPLOAD_DIR . '.htaccess',
        "Require all denied\nphp_flag engine off\nOptions -ExecCGI\n"
    );
...
if (!preg_match('/^[a-zA-Z0-9]{4,12}$/', $studentnummer)) {
    http_response_code(400);
    exit('Ongeldig studentnummer.');
}
...
// Ook in submap directe toegang blokkeren
@file_put_contents(
    $subdir . '.htaccess',
    "Require all denied\nphp_flag engine off\nOptions -ExecCGI\n"
);
